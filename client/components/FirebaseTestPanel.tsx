...
} from "@/utils/firestoreRules";
import {
  testFirebaseConnectivity,
  getNetworkEnvironmentInfo,
...
export function FirebaseTestPanel() {
  ...
  const runComprehensiveTest = async () => {
    ...
    try {
      ...
      console.log(
        "🏁 Comprehensive Firebase test completed:",
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="flex flex-wrap">
                      {getStatusBadge(
                        results.debug.read.success,
                        "Firestore Read",
                      )}
                      {getStatusBadge(
                        results.debug.write.success,
                        "Firestore Write",
                      )}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap">
                      {getStatusBadge(
                        results.network.firestore,
                        "Firestore Access",
                      )}
                      {getStatusBadge(results.network.auth, "Auth Access")}
                      {getStatusBadge(
                        results.network.storage,
                        "Storage Access",
                      )}
                    </div>
                  ...
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground space-y-1">
                      <div>
                        Project ID: <code>{results.projectInfo.projectId}</code>
                      </div>
                      <div>
                        Auth Domain:{" "}
                        <code>{results.projectInfo.authDomain}</code>
                      </div>
                  ...
                  {!results.overall && (
                    <div className="p-2 bg-yellow-50 dark:bg-yellow-900/20 rounded border border-yellow-200 dark:border-yellow-800">
                      <div className="text-xs text-yellow-800 dark:text-yellow-200">
                        <strong>Troubleshooting:</strong> If tests are failing,
                        check:
                        <ul className="mt-1 ml-4 list-disc">
                          <li>
                            Firebase project permissions and Firestore rules
                          </li>
