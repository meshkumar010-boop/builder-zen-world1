...
export function diagnoseNetworkIssues(results: NetworkTestResult[]): string[] {
  ...
  const firebaseServices = results.filter((r) =>
    r.service.includes("Firebase"),
  );
  const allFirebaseFailed = firebaseServices.every((r) => !r.success);

  if (allFirebaseFailed) {
    issues.push(
      "🚫 All Firebase services are unreachable - likely network/firewall issue",
    );
  }

  // Check for timeout issues
  const timeouts = results.filter(
    (r) => r.error?.includes("timeout") || r.responseTime > 10000,
  );
  if (timeouts.length > 0) {
    issues.push(
      "⏰ Slow network detected - consider checking connection speed",
    );
  }

  // Check for CORS issues
  const corsErrors = results.filter(
    (r) => r.error?.includes("CORS") || r.error?.includes("blocked"),
  );
  if (corsErrors.length > 0) {
    issues.push(
      "🔒 CORS/security policy blocking requests - check browser extensions",
    );
  }
  ...
  if (dnsTest && !dnsTest.success) {
    issues.push(
      "🌐 DNS resolution issues detected - check internet connection",
    );
  }
...
export async function testFirebaseConnectivity(): Promise<{
  ...
  return {
    firestore:
      firebaseResults.find((r) => r.service.includes("Firestore"))?.success ||
      false,
    auth:
      firebaseResults.find((r) => r.service.includes("Auth"))?.success || false,
    storage:
      firebaseResults.find((r) => r.service.includes("Storage"))?.success ||
      false,
