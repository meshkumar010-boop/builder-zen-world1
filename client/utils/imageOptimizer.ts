 ...
 */
export function needsOptimization(file: File, options: ImageOptimizationOptions = {}): boolean {
  const opts = { ...DEFAULT_OPTIONS, ...options };
  const fileSizeKB = file.size / 1024;
  
  return fileSizeKB > opts.maxSizeKB || file.type === 'image/png' || file.type === 'image/bmp';
}
...
export async function optimizeImage(
  ...
  try {
    console.log(`🖼️ Optimizing image: ${file.name} (${Math.round(originalSize / 1024)}KB)`);
    ...
    console.log(`✅ Image optimized: ${Math.round(originalSize / 1024)}KB → ${Math.round(optimizedSize / 1024)}KB (${Math.round(compressionRatio)}% reduction)`);
...
async function optimizeImageAggressive(
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      img.onload = () => resolve();
      img.onerror = () => reject(new Error('Failed to load image for aggressive compression'));
      img.src = URL.createObjectURL(file);
    });
    ...
    const blob = await new Promise<Blob>((resolve, reject) => {
      canvas.toBlob(
        (blob) => blob ? resolve(blob) : reject(new Error('Aggressive compression failed')),
        'image/jpeg',
        Math.max(0.3, options.quality - 0.3) // Reduce quality significantly
      );
    });
    
    const optimizedFile = new File([blob], generateOptimizedFilename(file.name, 'image/jpeg'), {
      type: 'image/jpeg'
    });
    ...
    console.log(`🗜️ Aggressive compression: ${Math.round(originalSize / 1024)}KB → ${Math.round(optimizedSize / 1024)}KB`);
...
export async function fileToOptimizedBase64(file: File): Promise<string> {
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      ...
      if (sizeKB > 200) { // Still too large for storage
        reject(new Error(`Image too large for storage: ${Math.round(sizeKB)}KB. Please choose a smaller image.`));
      } else {
        console.log(`📦 Base64 created: ${Math.round(sizeKB)}KB`);
        resolve(base64);
      }
    };
    reader.onerror = () => reject(new Error('Failed to convert optimized image to base64'));
 ...
 */
function generateOptimizedFilename(originalName: string, outputFormat: string): string {
 ...
 */
export function checkStorageCapacity(): { available: boolean; usageKB: number; message?: string } {
  try {
    ...
    if (usageKB > 4000) { // Approaching 5MB limit
      return {
        available: false,
        usageKB,
    ...
    return {
      available: false,
      usageKB: 0,
      message: 'Storage quota exceeded. Please delete some products to continue.'
    };
  }
}
